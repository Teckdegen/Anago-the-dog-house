      ...
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 flex-wrap">
            <p className="font-grotesk text-[20px] font-medium" style={{ color: "#FFFFFF" }}>
              {live.token0Symbol} / {live.token1Symbol}
            </p>
            <span className="font-mono text-[10px] px-2 py-1 rounded" style={{ background: "rgba(139,92,246,0.15)", color: "#A78BFA" }}>
              {feeToPercent(live.pool.fee)}
            </span>
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 mt-3 font-mono text-[10px]" style={{ color: "rgba(255,255,255,0.55)" }}>
            <div>
              <span className="opacity-60">Tick </span>
              <span style={{ color: "#FFFFFF" }}>{live.tick}</span>
            </div>
            <div>
              <span className="opacity-60">Liquidity </span>
              <span style={{ color: "#FFFFFF" }}>{live.liquidity.toString().slice(0, 14)}…</span>
            </div>
